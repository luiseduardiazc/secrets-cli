...
const { hashPassword, comparePassword, generateRandomKey, encrypt, decrypt, generateKey } = require('./crypto')
...
async function createSecret (username, password, name, value) {
  ...
  const secretKey = await generateKey(password)
  const encrypted = await encrypt(value, secretKey, randomKey)
...
async function getSecret (username, password, name) {
  ...
  const encrypted = resp.rows[0].value
  const randomKey = user.randomkey
  const secretKey = await generateKey(password)
  const decrypted = await decrypt(encrypted, secretKey, randomKey)
  resp.rows[0].value = decrypted
  return resp.rows[0]
}
